...
# Strategie MOMENTUM (continuation) faible rotation, intraday, 100% CAUSALE, flat EOD.
# Strategie FIGEE du projet (cf. diagnostics) : a 1-min les niveaux d'indices
# CONTINUENT -> on se positionne DANS le sens de l'ecart a la moyenne.
#
# Choix de conception (a defendre dans le rapport) :
#   1) FAIBLE ROTATION = le vrai levier identifie en IS. On l'obtient par un
#      seuil d'entree eleve (|z| > k_entry, k_entry=3 -> entrees rares) et par
#      le MAINTIEN de la position jusqu'a la sortie (pas de re-sizing barre par
#      barre, qui ajouterait de la rotation sans edge).
#   2) SIZING PROPORTIONNEL tanh (consigne Bloch "proportional sizing") : la
#      TAILLE de la position est w = tanh(gamma*z) au moment de l'ENTREE, bornee
#      dans (-1, 1), puis MAINTENUE constante jusqu'a la sortie. Conviction plus
#      forte (|z| grand) -> position plus grande, mais jamais > 1.
#   3) CAUSALITE : z_t n'utilise que le passe (rolling pandas). Flat a la
#      derniere barre -> aucune position overnight.
#
# Interface IDENTIQUE a strategy.compute_positions -> drop-in dans backtester.py
# (remplacer "import strategy" par "import strategy_momentum as strategy").
...
# --- Parametres FIGES de la strategie (calibrables en Phase 5, sur l'IS only) ---
